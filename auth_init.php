...
if(!empty($jsonObj->request_type) && $jsonObj->request_type == 'user_auth'){ 
    ...
    if(!empty($responsePayload)){ 
        ...
        $query = "SELECT id,oauth_provider,oauth_uid,first_name,last_name,email,picture,created,modified FROM users WHERE oauth_provider = '".$oauth_provider."' AND oauth_uid = '".$oauth_uid."'"; 
        ...
        if($result->num_rows > 0){  
            ...
            // Insert user data 
            $query = "INSERT INTO users (id, oauth_provider, oauth_uid, first_name, last_name, email, picture, created, modified) VALUES  (NULL, '".$oauth_provider."', '".$oauth_uid."', '".$first_name."', '".$last_name."', '".$email."', '".$picture."', NOW(), NOW())"; 
        ...
        $serializedData = json_encode($responsePayload);
        $_SESSION['jsonData'] = $serializedData;
